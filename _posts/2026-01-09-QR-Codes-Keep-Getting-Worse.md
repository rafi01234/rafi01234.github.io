...
Now onto where the research comes in. The researcher found that they could combine and overlay two distinct Model 2 QR codes on top of each other into a single image, with one acting as the primary (base) module and the overlay as the sub-module. 
